...
5. **방해하지 않는다** — 포커스를 뺏지 않고, **소리는 기본 꺼짐**, 알림은 보내지 않고,
   설정(펭귄 on/off·대사 목록·소리 on/off·음량·테마·핀볼·마릿수·크기) 말고는 아무것도
   저장하지 않는다.
...
- **`cargo fmt`은 크레이트 전체를 재포맷한다 — PR 범위를 조용히 넓힌다.** 새 파일 하나를
  정리하려고 돌렸는데 이 PR과 무관한 21개 파일이 딸려 왔다(#59). 두 러너·타입 검사가
  전부 통과하므로 diff를 직접 안 보면 드러나지 않고, "PR 하나는 체크박스 하나"가
  깨진다. 파일을 좁혀 돌리거나(`cargo fmt -- <파일>`), 돌린 뒤
  `git diff <base> --stat`으로 무엇이 딸려 왔는지 반드시 본다.
...
빈도 재조정, **핀볼 모드**(PRD §5.8), **효과음**(아홉에서만 — 합성이 원칙이고 안물의
목소리만 음원 파일이다,
